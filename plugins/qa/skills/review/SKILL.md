---
name: review
description: Ревью изменений с фиксированной точки (commit, branch, tag или merge-base) по двум осям — Standards (соответствует ли код задокументированным стандартам кодирования этого репозитория?) и Spec (соответствует ли код тому, что требовали исходные issue/PRD?). Запускает оба ревью в параллельных sub-agents и выводит их рядом. Используй, когда пользователь хочет проревьюить ветку, PR, незавершённые изменения или просит "review since X".
---
